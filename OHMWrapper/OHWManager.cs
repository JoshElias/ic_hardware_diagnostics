            ...
            try
            {
                ...
                foreach (var hardwareItem in myComputer.Hardware)
                {
                    ...
                    if (hardwareItem.Name.Contains("processor") || hardwareItem.Name.Contains("Processor"))
                        hardwareElement = new XElement("Processor");
                    else
                        hardwareElement = new XElement(HelperFunctions.CleanHardwareIdentifiers(hardwareItem.HardwareType.ToString()));
                        //hardwareElement = new XElement(HelperFunctions.ExceptChars(hardwareItem.HardwareType.ToString(), new[] { ' ', '/', '2' }));

                    //XAttribute name = new XAttribute("Name", HelperFunctions.ExceptChars(hardwareItem.Name, new[] { ' ', '/', '2' }));
                    //hardwareElement.Add(name);
